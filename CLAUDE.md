...
- **Read-only against the user's primary checkouts.** All speculative output goes to `git worktree`-based `prep/*` branches under `~/glean/work/` or to dossier dirs under `~/glean/dossiers/`. Never mutate the user's main checkout or push anything. **Boundary model (ADR-0009):** built-in Edit/Write are bounded to the worktree via `--add-dir`, and the default-deny allow-list grants no bare `Bash`; but an allow-listed *interpreter* verb (`node`, `npm run`, a test runner) spawns a subprocess OUTSIDE the permission layer, and native Windows has **no OS sandbox** — so for `draft-impl` this is **defense-in-depth, not a hard guarantee on Windows**. The default allow-list excludes `node`/`npm run`; `config.strict_spawn: true` drops in-session code execution entirely for a hard guarantee (the sandbox restores it safely on macOS/Linux/WSL2 — deferred).
- **Spawned sessions must run with a deny-list AND a scoped default-deny allow-list.** Every `claude -p` invocation passes `--disallowedTools "Bash(git push:*) Bash(git checkout main:*) Bash(gh pr merge:*) Bash(gh pr create:*)"`; `draft-impl`/`research-dossier` also pass a scoped `--allowedTools` (never bare `Bash`). Do not remove these even if a task seems to need them. The deny-list is in-session defense-in-depth; the allow-list (+ `--add-dir`) bounds which tools/writes are possible at all (ADR-0009).
...
- **Heuristic-only ranking for MVP.** `est_value` from discovery-evidence signal strength; `est_tokens` is a **coarse per-source constant** (≈2000–6000 by discovery source, hardcoded in `discover-*.ts` — *not* a per-candidate template/excerpt size estimate), so the `log(est_tokens+1)` denominator is near-inert within a type. No upfront `claude -p` triage pass unless rankings prove bad post-MVP.
