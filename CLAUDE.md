...
**v0.8.2 published to npm** (`@jonny-boy9000/glean@0.8.2`, merge `f1ff650`/PR #13, tag `v0.8.2`): drain robustness — configurable circuit-breaker + richer `productive` signal, first-class mid-weekend re-discovery + `evidence_hash` dedup, per-burst anti-spill margin (runDrain guard 3e), `today`/`peek` window-aware aggregation, and the honest rate-limit signal ([ADR-0001](./docs/decisions/0001-rate-limit-signal-source.md): parse verified `rate_limit_event.resetsAt`; stderr block detector unchanged; self-capturing BLOCK-CAPTURE tripwire). 402 tests + 2 documented skips. Built on v0.8.0 drain core + v0.8.1 UX polish. Public repo at https://github.com/Jonny-boy9000/glean. Install: `npm i -g @jonny-boy9000/glean` (the CLI command is still `glean`). See [`docs/PROJECT-MAP.md`](./docs/PROJECT-MAP.md) for the full layout. **To pick up cold, read [`docs/handoff/post-v0.8.2-handoff.md`](./docs/handoff/post-v0.8.2-handoff.md)** — the live forward handoff. **Next:** the one real overnight/weekend drain run to capture the true hard-block signal and close ADR-0001 (the BLOCK-CAPTURE tripwire makes it automatic), plus launch/adoption (demo GIF).
