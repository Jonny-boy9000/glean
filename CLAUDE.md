...
**v0.8.4 published to npm** (`@jonny-boy9000/glean@0.8.4`, PRs #15–#19, tag `v0.8.4`): the `glean serve` management dashboard (+ live capacity panel), Linux beta (systemd user timers), the two drain-resilience fixes from the 2026-06-11 live run (honest dedup ledger + structured-429 short-circuit, ADR-0003), dossier read-scope (ADR-0002 Accepted), and the docs consolidation. Built on v0.8.3 (schedule status/disable query fix) and **v0.8.2 drain robustness** (configurable circuit-breaker + richer `productive` signal, mid-weekend re-discovery + `evidence_hash` dedup, per-burst anti-spill margin, `today`/`peek` window-aware aggregation, honest rate-limit signal per [ADR-0001](./docs/decisions/0001-rate-limit-signal-source.md)). 406 tests + 2 documented skips. Public repo at https://github.com/Jonny-boy9000/glean. Install: `npm i -g @jonny-boy9000/glean` (CLI is still `glean`). See [`docs/PROJECT-MAP.md`](./docs/PROJECT-MAP.md). **Live-validated 2026-06-02:** first real run (4 tasks, 0 failed) + drain tick succeeded against `claude -p`; the unattended drain is **scheduled (fires Thu 2026-06-04 18:00)**. **To pick up cold, read [`docs/handoff/post-v0.8.2-handoff.md`](./docs/handoff/post-v0.8.2-handoff.md).** **Next:** ADR-0001 closes itself when the scheduled drain hits the weekly cap (BLOCK-CAPTURE tripwire is armed); then launch/adoption (demo GIF) + the fetch-docs-under-deny-list finding (see ROADMAP).
...
**Next milestone is v0.9: capacity governor** — `glean usage` (self-relative weekly pacing from local JSONL accounting), a nightly schedule preset gated by pace, model routing (`--model sonnet` default for drains — Sonnet has its own weekly pool on Max and burns the shared cap several times slower than Opus), and the two drain-resilience fixes from the 2026-06-11 live run (failed tasks wrongly dedup-ledgered as completed; no 429 short-circuit). Design: [`docs/design/2026-06-12-capacity-governor-strategy.md`](./docs/design/2026-06-12-capacity-governor-strategy.md). For "what's next" see **[`docs/ROADMAP.md`](./docs/ROADMAP.md)** — the single source of truth for planned work. Update it on every release.
