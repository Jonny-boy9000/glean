...
**v0.8.5 published to npm** (`@jonny-boy9000/glean@0.8.5`, PRs #20–#23, tag `v0.8.5`): the **project portfolio** (dashboard Projects tab mapping every Claude project on the machine, priority dials off/low/normal/high, add-project, `glean projects` CLI) plus three fixes found by the 2026-06-12 live multi-project test campaign — cwd-based session-history matching (slug encoding is ambiguous; 0→17 candidates on `C:\ClaudeCode_Work`), never-empty titles + unique dossier dirs (blank titles silently overwrote each other's OUT.md), and sleep-proof task deadlines + bounded kill grace (ADR-0004 — a "34.5-min timeout" was actually S3 sleep). Built on **v0.8.4** (PRs #15–#19: `glean serve` dashboard + capacity panel, Linux beta systemd user timers, honest dedup ledger + structured-429 short-circuit per ADR-0003, dossier read-scope per ADR-0002, docs consolidation) and v0.8.2/0.8.3 drain robustness ([ADR-0001](./docs/decisions/0001-rate-limit-signal-source.md), superseded by 0003). **538 tests + 6 skips (4 Linux-gated).** Public repo at https://github.com/Jonny-boy9000/glean. Install: `npm i -g @jonny-boy9000/glean`. See [`docs/PROJECT-MAP.md`](./docs/PROJECT-MAP.md). **To pick up cold, read [`docs/handoff/post-v0.8.2-handoff.md`](./docs/handoff/post-v0.8.2-handoff.md).**
...
**Next milestone is v0.9: capacity governor** (in progress 2026-06-13) — `glean usage` (self-relative weekly pacing from local JSONL accounting), a nightly schedule preset gated by pace, model routing (`--model sonnet` default for drains — Sonnet has its own weekly pool on Max and burns the shared cap several times slower than Opus), `--max-turns` guards, the `discover-docs` pass (mining roadmaps/handoffs as candidates), and dashboard auto-start at logon. Design: [`docs/design/2026-06-12-capacity-governor-strategy.md`](./docs/design/2026-06-12-capacity-governor-strategy.md) (adversarially reviewed; the pacing math is pinned in its "Pacing definition" section). For "what's next" see **[`docs/ROADMAP.md`](./docs/ROADMAP.md)** — the single source of truth for planned work. Update it on every release.
