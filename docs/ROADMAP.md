...
**Last updated:** 2026-06-23 (post-audit safety hardening — PRs #31/#32 merged on `main`; see [`docs/strategy/2026-06-23-hardening-roadmap.md`](./strategy/2026-06-23-hardening-roadmap.md))
**Current release:** **v0.10.0 published** (`@jonny-boy9000/glean@0.10.0`, tag `v0.10.0`). v0.10.0 = the moat wave: user-input **week anchor** (`pacing.week_anchor`), opt-in **morning anti-spill**, **nightly pace-gated** drain (`pace-skip`), budget-fit **in-run re-ranking** + adaptive type-downweighting, and the designed-but-unbuilt **API-backend seam** ([ADR-0008](./decisions/0008-spawn-backend-seam.md)). Direction pinned by the 2026-06-22 strategy review ([`docs/strategy/2026-06-22-next-wave-strategy.md`](./strategy/2026-06-22-next-wave-strategy.md): stay narrow / subscription-only / Claude-native). Built on **v0.9.0** (also published 2026-06-22: capacity-governor wave-1 + the full-project-review hardening — [`docs/reviews/2026-06-21-full-project-review.md`](./reviews/2026-06-21-full-project-review.md)). **850 tests + 8 skips** on `main` HEAD (v0.10.0 npm release was 801; PRs #31–#36 — the audit-remediation safety hardening, Phase 1 complete with ADR-0013 `enforce_spawn` + ADR-0010 `setup-token` auth — added the rest since).
**Branch state:** `main` is clean at **v0.10.0**. One WIP feature remains on an unmerged branch in a locked worktree: `feat/discover-docs-dirs` (discover-docs directory expansion / configurable doc globs — the Terra Firma follow-up below). `feat/nightly-mode` is **superseded** by the v0.10.0 nightly pace-gate and can be pruned once verified.
