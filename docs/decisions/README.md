...
| [0010](./0010-auth-failure-handling.md) | Auth-failure handling (detect + stop loudly, exit 50) + the `setup-token` scheduled-auth path | **Accepted** (detection + scheduled-auth path BUILT; auth-shape UNVERIFIED/capture-armed; live Spike-A re-validation tracked-pending) |
| [0011](./0011-tos-basis-for-scheduled-claude-p.md) | ToS basis for scheduled/unattended `claude -p` (official binary, Consumer-Terms §3; the unattended edge UNVERIFIED) | **Accepted** (basis recorded; unattended-vs-interactive leg UNVERIFIED; metered un-pause = existential tripwire) |
| [0012](./0012-conditional-economic-thesis.md) | The "free idle capacity" thesis is true but CONDITIONAL (shared pool; extra-usage overage) | **Accepted** (HOLDS-scoped; no engine change — pacing gates by design) |
| [0013](./0013-os-sandbox-enforcement.md) | OS-sandbox enforcement (`enforce_spawn`) — the HARD spawn boundary on mac/Linux/WSL2; Narrow fallback on Windows | **Accepted** (1a built+tested; live proof + 1b verb-restore TRACKED-PENDING off Windows). Supersedes the deferred sandbox leg of 0009 |
| [0014](./0014-honest-draft-test-status.md) | Honest draft test-status taxonomy (`env-blocked` ≠ `fail`, preamble-anchored; post-spawn node_modules link) | **Accepted** (built + tested). Supersedes the overloaded `none` producer token |
