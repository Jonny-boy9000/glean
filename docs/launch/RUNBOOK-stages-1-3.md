...
### What works today (dogfooded + tested, 814 tests)
...
How Claude helped: glean was built almost entirely with Claude Code — the discovery heuristics, the executor that wraps `claude -p`, the Windows Task Scheduler integration, and its 814 tests. And at runtime it *is* Claude Code: every unit of work is a headless `claude -p` session it spawns and supervises.
...
Built with: Claude Code, top to bottom — discovery heuristics, the executor that wraps `claude -p`, the Windows Task Scheduler integration, 814 tests. And at runtime it *is* Claude Code: every unit of work is a headless `claude -p` session it spawns and supervises.
