...
### Added
- **`glean usage` + the pacing engine — self-relative weekly pacing.** glean now reads its own token spend straight from `~/.claude/projects/**/*.jsonl` (`src/lib/usage.ts`, an **internal** loader — [ADR-0007](./docs/decisions/0007-internal-usage-loader.md) records why glean does *not* depend on `ccusage`: the programmatic API was dropped upstream in v19/v20 and its daily aggregation can't do glean's cwd-based own-session exclusion) and compares this week's cumulative weighted tokens against a per-weekday median baseline over the trailing 4 complete calendar weeks (`src/lib/pacing.ts`, pure + injectable clock; per-family weights pinned in [ADR-0005](./docs/decisions/0005-model-weight-multipliers.md)). `glean usage [--json]` prints the week-vs-baseline table, the pace ratio, the tier (skip / small / normal / large), the last captured five-hour utilization, and an honest blind-spot note (claude.ai web/desktop and other machines share the cap but write no local JSONL). Cold-start (<14 days of history) and a zero baseline are pinned conservative. `recommendTier()` is the wave-2 API the nightly schedule preset will consume.
- **`discover-docs` — a 4th parallel discovery pass.** Alongside JSONL session history, the git scan, and dependency changes, glean now mines the project's *own* planning docs (`ROADMAP`/`TODO`/`BACKLOG`/`PLAN`, `docs/ROADMAP.md`, `docs/handoff/*.md`, and root `*.md` whose first heading is planning-titled) for "up next" list items and unchecked `- [ ]` tasks, turning them into research-dossier candidates with a line-stable `evidence_hash`. Caps: 20 files / 200 KB / 10 candidates; `est_value` 28, just under the JSONL base of 30. Directly answers the 2026-06-11 drain's root cause — it ran out of *work*, not capacity.
...
- **`glean doctor` — environment preflight.** Checks Node ≥ 20, `claude` on PATH, git present, gh present (optional/non-fatal), `config.json` presence, and that `better-sqlite3` loads. Exits non-zero if a hard requirement is missing, so a misconfigured machine surfaces the problem once at the start instead of per-task at spawn time (where `--dry-run` would lull you).

### Fixed (2026-06-22 full-project review hardening)
- **`glean gc` leaked `prep/glean-*` branches forever.** `prepBranchFor` reconstructed the branch name with `lastIndexOf('-')`, so a real hyphenated UUID task id produced `prep/glean-<last-segment>` and `git branch -D` silently missed the actual `prep/glean-<full-uuid>` — the worktree was pruned but the branch leaked every time. gc now resolves the real branch authoritatively from `git worktree list --porcelain`. (Found by the review; the old gc test masked it by using a hyphen-free id.)
- **Cross-day INDEX dedup suppressed FAILED/timeout candidates for 7 days.** The in-burst ledger already excluded non-OK outcomes (ADR-0003), but `extractHashesFromIndex` applied no status filter, so a candidate that *failed* yesterday was treated as "done" and skipped for a week — genuinely-unfinished work silently dropped. Cross-day dedup now suppresses only successful outcomes, matching the in-burst ledger.
- **IPv6-loopback (`[::1]`) hole in the dashboard's DNS-rebinding host check.** The loopback-Host guard didn't recognize the bracketed IPv6 form, a latent gap in the anti-rebinding defense; now handled.

### Security
- **Hardened PowerShell scheduled-task registration against `day`/`time`/path injection.** Values flow from CLI flags and `config.json` (not the web surface, so this is defense-in-depth, not remote RCE), but `day` was interpolated raw and path escaping was `"`-only. The builders now whitelist `day` against weekday names, validate `time` against `^\d{2}:\d{2}$`, and properly marshal/escape interpolated values.

### Internal
- **`better-sqlite3` is now lazy-loaded.** The native binding was imported statically on the CLI's hot path, so a missing/unbuilt prebuilt killed *every* command — `glean version`, `glean doctor`, all of it. Memory is now loaded behind `await import` with a try/catch; a missing binding degrades to a one-line telemetry warning and the command runs.
- **Dropped the `uuid` dependency** in favor of Node's built-in `crypto.randomUUID`.
- **Split the 1070-line `executor.ts` god-file** into `executor.ts` (≈430 LOC orchestration) + `spawn-claude.ts` (the `claude -p` spawn state machine + rate-limit signal glue) + `draft-git.ts` (draft-impl git plumbing) + `draft-test.ts` (post-commit test runner). Behavior-preserving, done behind the existing `fn.impl` seams.
- Single-sourced `RATE_LIMIT_RE` (was duplicated verbatim in `executor.ts` and `classify.ts`, each with a "keep in sync" comment); one `homeDir()` helper (fixing a reversed HOME/USERPROFILE precedence); zod-validated `state/budget.json`; `tsconfig` moved to `NodeNext`; capped the vitest fork pool to de-flake the real-spawn suites.
- **New regression tests on the #1 safety boundary:** the deny-list is now asserted at the **argv** level on every spawn (research-dossier and draft-impl variants), not just as a constant, plus direct dashboard DNS-rebinding / loopback-Host tests.

## v0.8.5 — 2026-06-13 (PRs #20–#23, all found by the 2026-06-12 live multi-project test campaign)

### Added
- **Project portfolio — the dashboard's Projects tab** (PR #21): glean now maps out **every Claude project on the machine** (real paths extracted from the `cwd` field inside `~/.claude/projects` session files — the encoded dir name is never trusted; glean-spawn/worktree/temp noise filtered) unioned with configured projects, each with an **Off / Low / Normal / High priority dial** (discovered projects start Off and can never spend capacity until you opt in), an **Add project** form, and per-row Run buttons. CLI parity: `glean projects` + `glean projects set <path> <priority>`. The dial values feed the upcoming v0.9 multi-project allocator.

### Fixed
- **JSONL discovery was blind to projects with munged path characters** (PR #20). Claude Code encodes every special character to `-` in history-dir names (`C:\ClaudeCode_Work` → `C--ClaudeCode-Work`); glean computed its own slug and silently found zero sessions. History dirs are now matched by the `cwd` inside their session files. Live result: 0 → 17 candidates on the same project.
- **Blank-title candidates silently overwrote each other's dossiers** (PR #23). Sessions with no `ai-title`/`summary` entries produced empty titles → identical `research-/` dirs → each task destroyed the previous task's OUT.md (3 ran, 1 survived, live). Titles now resolve through a never-empty chain (`ai-title` → most recent `last-prompt` → first user message → `session <id>`), and dossier dirs are unique per task.
- **Per-task timeout enforcement survives system sleep and failed kills** (ADR-0004, PR #22). Live run `2026-06-12-1711-41b981`: an 8-min task recorded 34.5 min because the machine entered S3 sleep mid-task — the single `setTimeout` deadline cannot fire while asleep (and firing promptly on resume is platform luck), so the kill landed 26 minutes late and the 15-min budget overran to 37 min. The deadline is now polled against the wall clock (`Date.now()`, every 250 ms), so after any sleep/resume or clock jump the kill fires within ~a quarter second of the process being runnable again. Separately, a kill that fails to take the child down no longer wedges the executor forever: every kill (timeout and rate-limit alike) now carries a bounded grace (default 5 s, `ExecCtx.killGraceMs`) after which the spawn force-resolves with its real status, detaches the wedged pipes, and honestly reports `descendantsDead: false` so worktree lock cleanup (F7) stays safe. Regression-tested with a new `wedged.yaml` fake-claude scenario (emits an `api_retry`-shaped line forever, like the live failure) plus injectable kill/clock seams.
